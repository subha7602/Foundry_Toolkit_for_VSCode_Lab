...
"""
Personal Career Copilot - Multi-Agent Workflow Powered by Google Gemini.
...
logger = logging.getLogger("gemini-multi-agent")
...
def execute_step(client: OpenAI, instructions: str, user_content: str, use_tools: bool = False) -> str:
    ...
    kwargs = {"model": GEMINI_MODEL, "messages": messages, "temperature": 0.2}
    if use_tools:
        kwargs["tools"] = TOOLS
    ...
    if msg.tool_calls:
        ...
        final_resp = client.chat.completions.create(model=GEMINI_MODEL, messages=messages, temperature=0.2)
...
def start_server(host="127.0.0.1", port=8088):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            self._send_json({"status": "healthy", "agent": "PersonalCareerCopilot", "provider": "Gemini", "model": GEMINI_MODEL})
        ...
        def do_POST(self):
            try:
                ...
                logger.info(f"Processing prompt with Gemini ({GEMINI_MODEL}): {str(prompt)[:60]}...")
                ...
                if not GEMINI_API_KEY:
                    full_output = "Error: GEMINI_API_KEY is not set in your .env file."
                    send_delta(full_output)
                else:
                    ...
                    send_delta("Running multi-agent analysis on Gemini...\n\n[Step 1/4] Parsing candidate profile...\n")
    ...
    print("=" * 70)
    print(f"Gemini Multi-Agent Server running on http://{host}:{port}")
