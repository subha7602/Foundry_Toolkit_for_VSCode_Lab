...
def execute_step(client: Groq, instructions: str, user_content: str, use_tools: bool = False) -> str:
    messages = [
    ...
    if msg.tool_calls:
        ...
        final_resp = client.chat.completions.create(model=GROQ_MODEL, messages=messages, temperature=0.2)
...
def start_server(host="127.0.0.1", port=8088):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            self._send_json({"status": "healthy", "agent": "PersonalCareerCopilot", "provider": "Groq", "model": GROQ_MODEL})
