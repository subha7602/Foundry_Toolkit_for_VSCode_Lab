...
Lab 02: Sequential multi-agent pipeline (Resume Parser -> JD Analyst -> Matching Agent -> Gap Analyzer with Tool Calling).
"""
...
logger = logging.getLogger("groq-multi-agent")
...
Tool Usage (Required):
- For each major missing skill or certification gap, you MUST invoke the `search_learning_resources` tool.
...
def execute_agent_step(client: Groq, model: str, agent_name: str, system_prompt: str, user_content: str) -> str:
    """Executes a single conversational agent stage without tool calling."""
    logger.info(f"▶️ [{agent_name}] running on Groq ({model})...")
    ...
    time.sleep(0.5)  # smooth pacing between agent calls
    return response.choices[0].message.content or ""


def execute_gap_analyzer(client: Groq, model: str, system_prompt: str, user_content: str) -> str:
    """Executes the Gap Analyzer agent with tool calling loop."""
    logger.info(f"▶️ [GapAnalyzer] running with tool-calling on Groq ({model})...")
    start_time = time.time()
...
def run_career_copilot(resume_text: str, jd_text: str) -> dict:
    """Orchestrates the 4-agent sequential workflow."""
    client, model = create_groq_client()
    overall_start = time.time()

    print("\n" + "=" * 70)
    print(f" 🚀 Personal Career Copilot: 4-Agent Workflow on Groq ({model})")
    print("=" * 70)
    ...
    # Step 4: Gap Analyzer & Tool Calling
    ...
    print(f"\n✨ Workflow completed in {total_time:.2f} seconds total!")
...
"""
    ...
    print("=" * 70)
    print(" 🌟 Lab 02 - Multi-Agent Workflow: Resume -> Job Fit Evaluator")
    print("=" * 70)
