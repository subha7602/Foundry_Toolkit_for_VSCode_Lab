...
"""
Personal Career Copilot - Multi-Agent Workflow Powered by Azure AI Foundry.
Lab 02: Sequential 4-agent pipeline using Azure OpenAI / Azure AI Services with tool calling.
"""
...
logger = logging.getLogger("azure-foundry-multi-agent")
...
Tool Usage (Required):
- For major missing skills or certification gaps, you MUST invoke the `search_learning_resources` tool.
...
def search_learning_resources(skill: str) -> str:
    """Generates official learning documentation URLs and resources for a skill."""
...
def create_azure_client() -> tuple[AzureOpenAI, str]:
    """Initializes the Azure OpenAI client for Foundry."""
...
def execute_agent_step(client: AzureOpenAI, deployment: str, agent_name: str, system_prompt: str, user_content: str) -> str:
    """Executes a single conversational agent stage."""
    logger.info(f"▶️ [{agent_name}] running on Azure AI Foundry ({deployment})...")
...
def execute_gap_analyzer(client: AzureOpenAI, deployment: str, system_prompt: str, user_content: str) -> str:
    """Executes the Gap Analyzer agent with tool calling."""
    logger.info(f"▶️ [GapAnalyzer] running with tool-calling on Azure AI Foundry ({deployment})...")
    start_time = time.time()
...
def run_career_copilot(resume_text: str, jd_text: str) -> dict:
    """Orchestrates the 4-agent sequential workflow on Azure Foundry."""
    client, deployment = create_azure_client()
    overall_start = time.time()

    print("\n" + "=" * 70)
    print(f" ☁️ Personal Career Copilot: 4-Agent Workflow on Azure AI Foundry ({deployment})")
    print("=" * 70)
    ...
    parsed_resume_output = execute_agent_step(
        client, deployment, "ResumeParser", RESUME_PARSER_INSTRUCTIONS, combined_input
    )

    print("\n[Step 2/4] Analyzing job requirements...")
    jd_analysis_output = execute_agent_step(
        client, deployment, "JobDescriptionAgent", JOB_DESCRIPTION_INSTRUCTIONS, parsed_resume_output
    )

    print("\n[Step 3/4] Calculating fit score & skill alignment...")
    matching_output = execute_agent_step(
        client, deployment, "MatchingAgent", MATCHING_AGENT_INSTRUCTIONS, jd_analysis_output
    )

    print("\n[Step 4/4] Generating personalized roadmap with resource lookup...")
    final_roadmap = execute_gap_analyzer(
        client, deployment, GAP_ANALYZER_INSTRUCTIONS, matching_output
    )

    total_time = time.time() - overall_start
    print(f"\n✨ Workflow completed on Azure Foundry in {total_time:.2f} seconds total!")
...
"""
    ...
    print("=" * 70)
    print(" 🌟 Lab 02 - Multi-Agent Workflow: Azure AI Foundry Runner")
    print("=" * 70)
