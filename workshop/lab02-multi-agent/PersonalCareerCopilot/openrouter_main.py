...
Lab 02: Sequential 4-agent pipeline using OpenRouter to access Claude, DeepSeek, GPT-4o, or Llama 3.3.
"""
...
def execute_agent_step(client: OpenAI, model: str, agent_name: str, system_prompt: str, user_content: str) -> str:
    logger.info(f"▶️ [{agent_name}] running on OpenRouter ({model})...")
...
def execute_gap_analyzer(client: OpenAI, model: str, system_prompt: str, user_content: str) -> str:
    logger.info(f"▶️ [GapAnalyzer] running with tool-calling on OpenRouter ({model})...")
    start_time = time.time()
...
def run_career_copilot(resume_text: str, jd_text: str) -> dict:
    ...
    print("\n" + "=" * 70)
    print(f" 🌐 Personal Career Copilot: 4-Agent Workflow on OpenRouter ({model})")
    print("=" * 70)
    ...
    print(f"\n✨ Workflow completed on OpenRouter in {total_time:.2f} seconds total!")
...
"""
    ...
    print("=" * 70)
    print(" 🌟 Lab 02 - Multi-Agent Workflow: OpenRouter Runner")
    print("=" * 70)
